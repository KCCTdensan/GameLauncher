...
HomeScene::HomeScene(SharingScenes* _sharingScenes)
	...
	canvas(PosVec(0.f, ApplicationPreference::startScenePos), PosVec(ApplicationPreference::GetBackgroundSize().x, ApplicationPreference::GetBackgroundSize().y - ApplicationPreference::startScenePos), false),
	...
	works()
{
	...
	works.resize(lists.size(), ButtonObject(PosVec(), PosVec(), true, true));
